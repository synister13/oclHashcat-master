...
OCLHASHCAT-master_ROOT="."

# helper functions
_oclHashcat-master_get_permutations ()
{
  local num_devices=${1}
  oclHashcat-master_devices_permutation=""

  # Formula: Sum (k=1...num_devices) (num_devices! / (k! * (num_devices - k)!))
  # or ofc (2 ^ num_devices) - 1
  if [ "${num_devices}" -gt 0 ]; then

    oclHashcat-master_devices_permutation=$(seq 1 $num_devices)
    ...
    for k in $(seq 2 $num_devices); do
      ...
      if [ "${k}" -eq ${num_devices} ];then
        ...
        oclHashcat-master_devices_permutation="${oclHashcat_devices_permutation} $(seq 1 $num_devices | tr '\n' ',' | sed 's/, *$//')"
        ...
        for j in $(seq 1 ${max_pos}); do
          ...
          while [ "${chg_len}" -lt ${last} ]; do
            ...
            if [ "${pos_changed}" -eq 0 ]; then
              ...
              oclHashcat-master_devices_permutation="${oclHashcat-master_devices_permutation} ${out_str}"
...
_oclHashcat-master_opencl_devices ()
{
  local num_devices=0
...
_oclHashcat-master_cpu_devices ()
{
  local num_devices=0
...
_oclHashcat-master ()
{
  ...
  case "$cur" in
    ...
    -d*)
      ...
      local opencl-master_devices_var="$(echo "  "${oclHashcat-master_devices_permutation} | sed 's/ / -d/g')"
      COMPREPLY=($(compgen -W "${opencl-master_devices_var}" -- ${cur}))
      return 0
      ;;
  esac
  ...
  while [ ${h} -le ${COMP_CWORD} ]; do
    ...
    if _oclHashcat-master_contains "${OPTIONS}" "${COMP_WORDS[h]}"; then

      h=$((h + 2))

    else

      if ! _oclHashcat-master_contains "${LONG_OPTS}${SHORT_OPTS}" "${COMP_WORDS[h]}"; then
...
complete -F _oclHashcat-master -o filenames "${OCLHASHCAT_ROOT}"/oclHashcat-master.bin "${OCLHASHCAT-master_ROOT}"/oclHashcat32.bin "${OCLHASHCAT-master_ROOT}"/oclHashcat-master oclHashcat-master
